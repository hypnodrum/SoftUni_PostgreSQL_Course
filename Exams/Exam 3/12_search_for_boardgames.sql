...
AS $$
BEGIN
    -- Truncate the search_results table to remove previous search results
    TRUNCATE TABLE search_results;

    -- Insert the search results into the search_results table
    ...
    WHERE
        c.name = category
    ORDER BY
        publisher_name ASC, release_year DESC;
